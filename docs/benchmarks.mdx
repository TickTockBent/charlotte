...
description: Head-to-head benchmarks comparing Charlotte and Playwright MCP on orientation cost, per-task cost, drift, and where Charlotte loses.
icon: chart-line
---
...
<Info>
  Live pages drift. Hacker News, Wikipedia, and GitHub content change day to day, so these numbers include real content drift alongside Charlotte's own changes. See the linked methodology for the same-day rule and per-version Chromium caveats.
</Info>
