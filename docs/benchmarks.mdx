...
What an agent pays to "see" a page on arrival. A Charlotte `navigate` returns a usable orientation by default — landmarks, headings, and interactive element counts grouped by page region. The Playwright MCP equivalent is `browser_snapshot`, which returns the full accessibility tree (`browser_navigate` alone returns only a short confirmation, not page content, so it isn't a like-for-like comparison).
...
Charlotte's orientation cost has stayed within a narrow band across seven releases while the tool surface was cut roughly in half (v0.3.0's 36 tools down to v0.8.0's 23); Playwright's baseline sits one to two orders of magnitude higher throughout because its `browser_snapshot` scales with page *content*, not page *structure*. See the drift report for the live-page-drift and Chromium-version caveats that qualify these numbers.
