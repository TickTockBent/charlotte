...
Week of 2026-08-21: persistent init scripts, named device emulation in `charlotte_viewport`, a privacy page and cookieless analytics on the marketing site, plus faster in-site navigation, and a server fix for selector-mode (`dom-`) element IDs in `charlotte_toggle` and the `charlotte_find` spatial filters.
