...
Week of 2026-08-21: named device emulation in `charlotte_viewport`, a privacy page and cookieless analytics on the marketing site, plus faster in-site navigation, and a server fix for selector-mode (`dom-`) element IDs in `charlotte_toggle` and the `charlotte_find` spatial filters.

### Fixed

- **Selector-mode `dom-` IDs in `charlotte_toggle` and `charlotte_find` filters** ([#220](https://github.com/TickTockBent/charlotte/issues/220)). IDs returned by `charlotte_find` in selector mode live outside the accessibility-derived `interactive` list, and three code paths assumed they did not:
  - `charlotte_toggle` skipped its toggleable-control guard for `dom-` IDs and fired an unvalidated click on whatever the ID pointed at (a link would be followed). It now classifies `dom-` targets live via CDP and rejects non-checkbox/radio/switch elements with `INVALID_ARGUMENT`, matching the behavior for observe-derived IDs.
  - The `near` and `within` filters on `charlotte_find` reported a misleading "has no bounds" error for a `dom-` reference. They now read the reference's live box model, so a `dom-` anchor works. An unknown anchor now reports `ELEMENT_NOT_FOUND` with a similar-ID suggestion.
  - `charlotte_find` recorded a `dom-` element's match index as the count of successfully described matches rather than its raw `querySelectorAll` position. Any earlier undescribable match (e.g. a pseudo-element) then shifted every later ID onto the wrong node on re-resolution. The stored index is now the raw position.
