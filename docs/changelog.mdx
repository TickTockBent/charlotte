...
Week of 2026-08-21: a privacy page and cookieless analytics on the marketing site, faster in-site navigation, and a fix for selector-mode (`dom-`) element IDs on `charlotte_toggle` and `charlotte_find`.

### Added

- **Privacy page and cookieless analytics** on the [Charlotte site](https://charlotte.mintlify.site/). Traffic data is aggregate and cookieless. No personal data is collected and no third-party trackers are loaded. A new **Privacy** link now appears in the shared footer, the changelog's inline footer, and the sitemap.

### Changed

- **Faster navigation on the marketing site.** Internal links in the homepage nav and shared footer now switch pages without a full reload. Moving between the homepage, [changelog](https://charlotte.mintlify.site/changelog), [benchmarks](https://charlotte.mintlify.site/benchmarks), and the new privacy page is instant. Hash anchors and external links are unchanged.

### Fixed

- **Selector-mode (`dom-`) element IDs on `charlotte_toggle` and `charlotte_find`** ([#220](https://github.com/TickTockBent/charlotte/issues/220)). `dom-` IDs returned by `charlotte_find` in selector mode were mishandled in three places:
  - `charlotte_toggle` now validates that a `dom-` target is a checkbox, radio, or switch before clicking, and rejects other elements instead of firing an unvalidated click.
  - The `near` and `within` filters on `charlotte_find` now accept a `dom-` reference, and report `ELEMENT_NOT_FOUND` (with a similar-ID suggestion) for unknown anchors instead of a misleading "has no bounds" error.
  - `charlotte_find` now records a `dom-` element's match index correctly, so results stay stable across re-resolution on pages with undescribable earlier matches.
