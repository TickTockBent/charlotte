...
description: "Full release notes and versioned history for Charlotte MCP server: new features, fixes, and unreleased changes on main from v0.1.0 onward."
icon: clock-rotate-left
---

This is Charlotte's canonical changelog — every release from v0.1.0 through the current version, plus unreleased changes on `main`. Tagged releases with published artifacts live on [GitHub Releases](https://github.com/TickTockBent/charlotte/releases).

## [Unreleased]

Week of 2026-09-11: dependency maintenance. The test runner moves to vitest 4.1.11 (with its coverage provider in lockstep) to clear a moderate advisory, transitive `js-yaml`, `hono`, and `@humanfs/node` advisories are cleared through lockfile bumps, the marketing site takes the Next.js 16.3.5 patch, and a second advisory ID for the already-accepted `extract-zip` risk is documented. No behavior changes.

### Security

- **Cleared: GHSA-2883-xcg3-v3hh (`js-yaml` 4.3.1).** CPU exhaustion in merge-key handling, transitive through `puppeteer` → `cosmiconfig`. Lockfile bump to 4.3.2; no source change.
- **Cleared: GHSA-82fw-gwwq-j7x9 (`@vitest/mocker` < 4.1.11).** Dev-only. `vitest` and `@vitest/coverage-v8` move together to 4.1.11 (the coverage provider pins its vitest peer exactly, so the pair cannot move independently). Vitest 5 is deferred to its own verified slice.
- **Cleared: moderate `hono` and `@humanfs/node` advisories** via lockfile bumps to 4.13.7 and 0.16.8 in both the server and the site.
- **Documented accepted risk: GHSA-7pqw-9j4j-h8q3 (`extract-zip` ≤ 2.0.1).** A second advisory ID for the same symlink archive-entry write already accepted as GHSA-jmr9-qjv8-65gv: same package, same version, same install-time-only exposure, still no patched release. Added to the audit allowlist under the existing rationale and listed under **Known accepted risks** in [Security](https://charlotte.mintlify.site/security); clears with the same Puppeteer 25 upgrade.

Week of 2026-08-21: persistent init scripts, named device emulation in `charlotte_viewport`, `linux/arm64` Docker images, a privacy page and cookieless analytics on the marketing site, plus faster in-site navigation, and a server fix for selector-mode (`dom-`) element IDs in `charlotte_toggle` and the `charlotte_find` spatial filters.

### Added

- **Persistent init scripts** ([#18](https://github.com/TickTockBent/charlotte/issues/18)). Charlotte's equivalent of Playwright's `--init-script`: JavaScript that runs on every new document in every tab, before any page script, via Puppeteer's `evaluateOnNewDocument`. Configure at startup with `browser.initScripts` in the config file (relative paths resolve against the config file's directory), `--init-script <path>` (repeatable), or `CHARLOTTE_INIT_SCRIPT` (several paths separated by the OS path delimiter); files are read once at startup and a missing file fails startup with the path named. At runtime, `charlotte_dev_inject` gains `persist: true`, which re-applies the given CSS or JS on every future page load for the rest of the session (CSS is wrapped to append a `<style>` once the document has a head). Popups' very first document is not covered — Puppeteer resumes a new target before Charlotte can attach to it — but every document after it is. See [Init scripts](https://charlotte.mintlify.site/configuration#init-scripts).
- **Named device emulation in `charlotte_viewport`** ([#24](https://github.com/TickTockBent/charlotte/issues/24)). `device` now accepts any Puppeteer `KnownDevices` name (e.g. `"iPhone 15"`, `"iPhone 15 landscape"`, `"Pixel 5"`, `"iPad Pro"`), matched case-insensitively, alongside the generic `mobile`/`tablet`/`desktop` presets. A named device applies the full descriptor via `page.emulate()`: viewport, device pixel ratio, touch support, mobile mode, and user agent. The response then carries an `emulation` object (`device`, `user_agent`, `device_scale_factor`, `has_touch`, `is_mobile`). Switching back to a generic preset or custom `width`/`height` resets DPR, touch, and mobile mode and restores the browser's default user agent, so emulation does not leak between calls. Unknown names fail with `INVALID_ARGUMENT` and up to five close `KnownDevices` suggestions.
- **`linux/arm64` Docker images** ([#22](https://github.com/TickTockBent/charlotte/issues/22)). Both the Debian (`:latest`) and Alpine images are now published as multi-arch manifests for `linux/amd64` and `linux/arm64`, so Apple Silicon Macs and ARM servers can run them natively instead of under emulation. **The arm64 variants are cross-built under QEMU and untested on real ARM hardware** — please report results in the issue. Chrome for Testing has no Linux arm64 build, so the Debian arm64 variant uses Debian's `chromium` package and the entrypoint points Puppeteer at it; amd64 images are unchanged and remain the smoke-tested platform. See [Supported platforms](https://charlotte.mintlify.site/docker#supported-platforms).
- **Privacy page and cookieless analytics** on the [Charlotte site](https://charlotte.mintlify.site/). Vercel Web Analytics is enabled at the site root and collects aggregate, cookieless traffic data only. No personal data is collected and no third-party trackers are loaded. A new **Privacy** link now appears in the shared footer, the changelog's inline footer, and the sitemap.

### Changed

- **Client-side navigation on the marketing site.** Internal links in the homepage nav and shared footer now route through Next.js client-side navigation instead of full page reloads. Moving between the homepage, [changelog](https://charlotte.mintlify.site/changelog), [benchmarks](https://charlotte.mintlify.site/benchmarks), and privacy pages is instant. Hash anchors and external links are unchanged.

Week of 2026-08-18: a screenshot ergonomics improvement for very long pages, and a documented security exception for a transitive install-time advisory. No breaking changes.
...
- **JSON configuration file** via `--config <path>` (and an auto-loaded `charlotte.config.json` in the working directory). Settings resolve with precedence **CLI args > env vars > config file > defaults**, validated with zod; an invalid config produces a clear startup error on stderr and a non-zero exit. New env vars: `CHARLOTTE_NO_SANDBOX`, `CHARLOTTE_OUTPUT_DIR`, `CHARLOTTE_CDP_ENDPOINT`. See [Configuration](https://charlotte.mintlify.site/configuration). (#19)
