---
title: Docker
description: Running Charlotte in Docker — image choices, sandbox posture, and production notes.
icon: docker
---

{/*
  Canonical source: DOCKER.md at the repo root. Changes flow
  canonical → this page, not the reverse — update DOCKER.md first,
...
- **`Dockerfile`** (Debian) — the **HTTP self-host image**. Runs Charlotte Remote
  ...
  tunnel setup): see [Self-Hosting](/self-hosting).
...
<CodeGroup>
```bash Debian, HTTP mode (self-host)
docker build -f Dockerfile -t charlotte:http .
docker compose up charlotte
```

```bash Alpine, stdio mode (local/dev, ~1.2GB)
...
<Note>
  Your mounted `charlotte.config.json` needs at least `http.host: "0.0.0.0"` so the server is reachable from outside the container (the code default, `127.0.0.1`, is loopback-only — see [Sandbox / Security Posture](#sandbox-/-security-posture) for why that default exists and [Self-Hosting](/self-hosting) for the full config). See `docker-compose.yml` for the same invocation with comments on the seccomp/SYS_ADMIN tradeoff.
</Note>
...
<Steps>
  <Step title="Surgical seccomp profile (default, recommended)">
  ...
  <Step title="cap_add: [SYS_ADMIN] with default seccomp (fallback)">
  ...
  <Step title="No relaxation needed">
...
<Danger>
  ...
  WARN about it on purpose (see [Configuration](/configuration) for the flag).
</Danger>
...
<AccordionGroup>
  <Accordion title="Failed to launch the browser process">
    - Ensure `--shm-size=2gb` is set
    - Debian/HTTP image: make sure you passed the seccomp profile or SYS_ADMIN fallback — see [Sandbox / Security Posture](#sandbox-/-security-posture); `--security-opt seccomp=unconfined` works too but is a much broader relaxation than this image needs
  ...
  <Accordion title="Chromium version mismatch (Alpine only)">
