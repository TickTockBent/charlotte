...
// ─── charlotte_dev_serve ───
const devServeTool = defineTool({
  name: "charlotte_dev_serve",
  description:
  ...
  async handler(deps, { path: directoryPath, port, watch }) {
    try {
...
// ─── charlotte_dev_audit ───
const devAuditTool = defineTool({
  name: "charlotte_dev_audit",
  description:
...
export const devModeTools: ToolDefinition[] = [devServeTool, devInjectTool, devAuditTool];
