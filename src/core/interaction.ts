...
import { waitForTools } from "./wait-for.js";
...
const clickTool = defineTool({
  ...
  async handler(deps, { element_id, click_type, modifiers }) {
    try {
...
// ─── charlotte_click_at ───
const clickAtTool = defineTool({
  name: "charlotte_click_at",
  description:
  ...
  async handler(deps, { x, y, click_type, modifiers }) {
    try {
      await ensureReady(deps);
...
const typeTool = defineTool({
  ...
  inputSchema: {
    ...
    press_enter: coercedBoolean.optional().describe("Press Enter after typing (default: false)"),
  ...
  async handler(deps, { element_id, text, clear_first, press_enter, slowly, character_delay }) {
    try {
...
const selectTool = defineTool({
  ...
  async handler(deps, { element_id, value }) {
    try {
      await ensureReady(deps);
      const resolved = await resolveElement(deps, element_id);
      const session = await getSessionForElement(deps, resolved);
...
// ─── charlotte_toggle ───
const toggleTool = defineTool({
  name: "charlotte_toggle",
  description:
...
const scrollTool = defineTool({
  ...
  async handler(deps, { direction, amount, element_id }) {
    try {
      await ensureReady(deps);
      const page = deps.pageManager.getActivePage();
...
const dragTool = defineTool({
  ...
  async handler(deps, { source_id, target_id }) {
    try {
      await ensureReady(deps);
...
const keyTool = defineTool({
  ...
  async handler(deps, { key, keys, modifiers, element_id, delay }) {
    try {
...
const uploadTool = defineTool({
  ...
  async handler(deps, { element_id, paths }) {
    try {
      await ensureReady(deps);
      const resolved = await resolveElement(deps, element_id);
      const session = await getSessionForElement(deps, resolved);
