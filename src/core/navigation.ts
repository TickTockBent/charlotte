...
const navigateTool = defineTool({
  ...
  async handler(deps, { url, wait_for, timeout, detail }) {
    try {
...
const backTool = defineTool({
  name: "charlotte_back",
  description: "Navigate back in browser history. Returns page representation after navigation.",
  inputSchema: {
    detail: detailSchema,
  },
  async handler(deps, { detail }) {
    try {
      await ensureReady(deps);
      const page = deps.pageManager.getActivePage();
...
const forwardTool = defineTool({
  name: "charlotte_forward",
  description: "Navigate forward in browser history. Returns page representation after navigation.",
  inputSchema: {
    detail: detailSchema,
  },
  async handler(deps, { detail }) {
...
const reloadTool = defineTool({
  ...
  async handler(deps, { hard, detail }) {
    try {
      await ensureReady(deps);
      const page = deps.pageManager.getActivePage();
...
export const navigationTools: ToolDefinition[] = [navigateTool, backTool, forwardTool, reloadTool];
