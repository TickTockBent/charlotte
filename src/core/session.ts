...
const getCookiesTool = defineTool({
  name: "charlotte_get_cookies",
  description:
  ...
  inputSchema: {
    urls: z
      .array(z.string())
      .optional()
      .describe("URLs to filter cookies by. If omitted, returns cookies for the current page URL."),
  },
  async handler(deps, { urls }) {
    try {
...
const clearCookiesTool = defineTool({
  name: "charlotte_clear_cookies",
  description:
...
const setCookiesTool = defineTool({
  name: "charlotte_set_cookies",
  description:
...
const setHeadersTool = defineTool({
  name: "charlotte_set_headers",
  description:
...
// ─── charlotte_tab_open ───
const tabOpenTool = defineTool({
  name: "charlotte_tab_open",
  description:
...
const tabSwitchTool = defineTool({
  name: "charlotte_tab_switch",
  description:
...
// ─── charlotte_tab_close ───
const tabCloseTool = defineTool({
  name: "charlotte_tab_close",
  description:
...
const networkTool = defineTool({
  ...
  async handler(deps, { throttle, block }) {
    try {
      await ensureReady(deps);
      const page = deps.pageManager.getActivePage();
