...
export async function checkBrowser(resolved: ResolvedOptions): Promise<DoctorCheckResult> {
  ...
  try {
    ...
    try {
      ...
      if (resolved.http && resolved.noSandbox) {
        return warn(
          ...
          "Remote + unsandboxed Chromium is a hardening regression (design spec §3.5): a hostile page now exploits the renderer on your server, not the visitor's machine. Only run --no-sandbox / CHARLOTTE_NO_SANDBOX in a container with a working seccomp profile (see docs/sandbox.md).",
        );
      }
      return pass(
        "Browser launches & renders",
    ...
    return fail(
      ...
      "If running in a container, pass --no-sandbox (or CHARLOTTE_NO_SANDBOX=1) and make sure a seccomp profile / userns setup is available (see docs/sandbox.md and SELF_HOSTING.md); on bare metal, check that Chromium's dependencies are installed.",
    );
  } finally {
